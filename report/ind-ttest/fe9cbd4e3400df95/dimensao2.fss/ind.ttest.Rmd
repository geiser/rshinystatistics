...
```{r echo=FALSE}
kdf <- normality_test_per_group(sdat, "dimensao2.fss", "cenário")
kdf$p <- round(kdf$p, 3)
...
```{r echo=FALSE}
kdf <- homogeneity_test(sdat, "dimensao2.fss", "cenário")
kdf$p <- round(kdf$p, 3)
kdf$p[which(kdf$p < 0.001)] <- '<0.001'
kable(kdf, digits = 3)
```
...
```{r echo=FALSE}
kdf <- descriptive_statistics(sdat, "dimensao2.fss", "cenário", "common")
