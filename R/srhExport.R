...
srhExportUI <- function(id) {
  ...
  verticalLayout(
    ...
    fixedRow(
      ...
      column(width = 2, actionButton(ns("exportSRH"), "Generate Export Files", icon = icon('file-export'))),
...
#' @import shiny
srhExportMD <- function(id, dataset, dvs = "dvs", between = "between") {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      path <- reactiveVal(
        paste0(getwd(),'/report/srh/',reportId())
      )
      ...
      updateFiles <- function(progress) {
        ...
        for (dv in input$dvs) {
          for (nfile in input$files) {
            ...
            rmarkdown::render(paste0(path(),'/',dv,'/srh.Rmd'), paste0(nfile,'_document'))
          }
        }
      }

      observeEvent(input$exportSRH, {
        ...
        progress$set(message = tl("Making files to export SRH"), value = 0)
      ...
      output$downloadButtonUI <- renderUI({
        if (!is.null(input$exportSRH) && (input$exportSRH) > 0) {
