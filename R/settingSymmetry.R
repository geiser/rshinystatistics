...
#' @import shiny
settingSymmetryMD <- function(id, dataset, dvs = "dvs", covar = 'covar', initTable = 'initTable', dataTable = 'dataTable') {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$skewnessInputUI <- renderUI({
        ...
        skewnessInputs <- lapply(c(rdvs(), rcovar()), FUN = function(dv) {
          ...
          names(choices) <- c(tl("None"), "sqrt(x)", "sqrt(max(x+1)-x)","log10(x)","log10(max(x+1)-x)","1/x","1/(max(x+1)-x)")
