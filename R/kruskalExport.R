...
kruskalExportUI <- function(id) {
  ...
  verticalLayout(
    ...
    fixedRow(
      ...
      column(width = 2, actionButton(ns("exportKruskal"), "Generate Export Files", icon = icon('file-export'))),
...
#' @import shiny
kruskalExportMD <- function(id, dataset, dvs = "dvs", between = "between") {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      path <- reactiveVal(
        paste0(getwd(),'/report/kruskal/',reportId())
      )
      ...
      updateFiles <- function(progress) {
        ...
        backup[["rds.signature"]] <- paste0('kruskal-',as.character(packageVersion("rshinystatistics")))
        ...
        cat(kruskalSummaryAsFile('R', backup, dvs, between, path = path()), file = paste0(path(), '/kruskal.R'))
        cat(kruskalSummaryAsFile('Rmd', backup, dvs, between), file = paste0(path(), '/summary.Rmd'))
        cat(kruskalSummaryAsFile('Rmd', backup, dvs, between, lang='pt'), file = paste0(path(), '/summary-pt.Rmd'))
        for (dv in rdvs()) write.csv(backup$initTable[[dv]], paste0(path(), '/data-',dv,'.csv'))

        for (dv in input$dvs) {
          dir.create(paste0(path(),'/',dv), showWarnings = F, recursive = T)
          cat(kruskalDetailAsFile('R', backup, dv, between, path = paste0(path(),'/',dv)), file = paste0(path(),'/',dv,'/kruskal.R'))
          cat(kruskalDetailAsFile('Rmd', backup, dv, between), file = paste0(path(),'/',dv,'/kruskal.Rmd'))
        ...
        for (dv in input$dvs) {
          for (nfile in input$files) {
            ...
            rmarkdown::render(paste0(path(),'/',dv,'/kruskal.Rmd'), paste0(nfile,'_document'))
          }
        }
      }

      observeEvent(input$exportKruskal, {
        if (!dataset$isSetup) return(NULL)
        validate(
          need(!is.null(dataset$kruskalParams[["hypothesis"]]),
               tl("Please perform the Kruskal-Wallis test before to generate the files to be exported"))
        )
        progress <- shiny::Progress$new()
        progress$set(message = tl("Making files to export Kruskal-Wallis"), value = 0)
      ...
      output$downloadButtonUI <- renderUI({
        if (!is.null(input$exportKruskal) && (input$exportKruskal) > 0) {
