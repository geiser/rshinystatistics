...
ggPlotAoC <- function(data, x, color = c(), aov, pwc, linetype = color, font.label.size = 10, step.increase = 0.005) {
  ...
  if (color != "black")
    ...
    lp <- lp + ggplot2::geom_errorbar(ggplot2::aes_string(ymin="conf.low", ymax="conf.high"), width=0.1)


  lp <- lp + ggpubr::stat_pvalue_manual(pwc2, hide.ns=T, tip.length=0.005, bracket.size=0.3, linetype=linetype, color=color)
