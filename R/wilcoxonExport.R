...
#' @import shiny
wilcoxonExportMD <- function(id, dataset, dvs = "dvs", iv = "iv", initTable='initTable', dataTable = 'dataTable', fileTable='fileTable') {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      reportId <- reactiveVal(
        digest::digest(list(
          ...
          wilcoxonParams = dataset$wilcoxonParams, all.names = T), algo = "xxhash64")
      )

      path <- reactiveVal(
        paste0(getwd(),'/report/wilcoxon/',reportId())
      )
      ...
      updateFiles <- function(progress) {
        ...
        # ... saving code
        cat(wilcoxonAsFile('Rmd', backup, dvs, iv), file = paste0(path(),'/code/wilcoxon.Rmd'))
        knitr::purl(paste0(path(),'/code/wilcoxon.Rmd'), paste0(path(),'/code/wilcoxon.R'), documentation = 2)

        # ... generating reports
        for (nfile in input$files) {
          progress$inc(inc, detail = paste('Generating', nfile,'files for the report'))
          rmarkdown::render(paste0(path(), '/code/wilcoxon.Rmd'), paste0(nfile,'_document'), output_dir = paste0(path(),'/results'))
        }
      }

      observeEvent(input[[paste0("exportWilcoxonTest",reportId())]], {
      ...
      output$exportButtonUI <- renderUI({
        if (!dataset$isSetup) return(NULL)
        actionButton(ns(paste0("exportWilcoxonTest",reportId())), tl("Generate Export Files"), icon = icon('file-export'))
      })

      output$downloadButtonUI <- renderUI({
        if (!is.null(input[[paste0("exportWilcoxonTest",reportId())]]) && (input[[paste0("exportWilcoxonTest",reportId())]]) > 0) {
