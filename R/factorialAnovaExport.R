...
#' @import shiny
factorialAnovaExportMD <- function(id, dataset, dvs = "dvs", between = "between", initTable='initTable', dataTable = 'dataTable', fileTable='fileTable') {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      reportId <- reactiveVal(
        digest::digest(list(
          ...
          anovaParams = dataset$anovaParams, dvs = dvs, between = between), algo = "xxhash64")
      )
      ...
      updateFiles <- function(progress) {
        ...
        # ... saving code
        cat(factorialAnovaSummaryAsFile('Rmd', backup, dvs, between), file = paste0(path(),'/code/factorialAnova.Rmd'))
        knitr::purl(paste0(path(),'/code/factorialAnova.Rmd'), paste0(path(),'/code/factorialAncova.R'), documentation = 2)

        # ... generating reports
        for (nfile in input$files) {
          progress$inc(inc, detail = paste('Generating', nfile,'files for the report'))
          rmarkdown::render(paste0(path(), '/code/factorialAnova.Rmd'), paste0(nfile,'_document'), output_dir = paste0(path(),'/results'))
        }
      }

      observeEvent(input[[paste0("exportFactorialAnova",reportId())]], {
        if (!dataset$isSetup) return(NULL)
        validate(
          need(!is.null(dataset$anovaParams[["hypothesis"]]),
               tl("Please perform the factorial ANOVA test before to generate the files to be exported"))
        )
        progress <- shiny::Progress$new()
        progress$set(message = tl("Making files to export factorial ANOVA"), value = 0)
      ...
      output$exportButtonUI <- renderUI({
        if (!dataset$isSetup) return(NULL)
        actionButton(ns(paste0("exportFactorialAnova",reportId())), tl("Generate Export Files"), icon = icon('file-export'))
      })

      output$downloadButtonUI <- renderUI({
        if (!is.null(input[[paste0("exportFactorialAnova",reportId())]]) && (input[[paste0("exportFactorialAnova",reportId())]]) > 0) {
