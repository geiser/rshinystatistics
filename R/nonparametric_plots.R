...
#' @export
ggPlotFactNonParam <- function(data, x, y, color = c(), non, pwc, linetype = color, by = c(), addParam = c(), font.label.size = 10, step.increase = 0.005, type = NULL) {
...
#' @export
threeWayNonParamFactPlots <- function(data, dv, ivs, non, pwcs, addParam=c(), font.label.size = 10, step.increase = 0.005, type = NULL) {
...
#' @export
twoWayNonParamFactPlots <- function(data, dv, ivs, non, pwcs, addParam=c(), font.label.size = 10, step.increase = 0.005, type = NULL) {
  livs <- as.list(ivs); names(livs) <- ivs
...
#' @export
oneWayNonParamFactPlots <- function(data, dv, ivs, non, pwcs, addParam=c(), font.label.size = 10, step.increase = 0.005, type = NULL) {
  livs <- as.list(ivs); names(livs) <- ivs
  return(lapply(livs, FUN = function(iv) {
