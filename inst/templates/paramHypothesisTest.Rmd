...
{{
if (length(covar) > 0)
  paste0('df <- dat; df[[covar]] <- dat[[1]]')
}}
(df <- descriptive_statistics(df, dvs, between, include.global = T, symmetry.test = T))
```
...
{{
if (length(covar) > 0) {
  paste0('(df <- normality_test_by_res(sdat, dvs, between, c(), covar))')
} else {
  paste0('(df <- normality_test_by_res(sdat, dvs, between)')
}
}}
```
...
```{r, include=FALSE}
(df <- descriptive_statistics(sdat, dvs, between, include.global = F, normality.test = T))
```
...
{{
if (length(covar) > 0) {
  paste0('(df <- homogeneity_test(sdat, dvs, between, c(), covar))')
} else {
  paste0('(df <- homogeneity_test(sdat, dvs, between)')
}
}}
```
