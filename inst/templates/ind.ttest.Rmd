---
title: "Independent t-test for {{ paste0(paste0('`', dvs, '`'), collapse = ',') }} with `{{ iv }}`"
author: {{ author }} <{{ email }}>
...
* Table without outliers:  [../data/table-with-normal-distribution.csv](../data/table-without-outliers.csv)
...
```{r, include=FALSE}
(df <- normality_test_by_res(sdat, dvs, iv))
```
...
```{r, include=FALSE}
(df <- homogeneity_test(sdat, dvs, iv))
```
...
## Computation of T-test and Pairwise Comparison

### T-test

```{r, include=FALSE}
(res <- ind_ttest(sdat, dvs, iv, "{{ alternative }}", {{ var.equal }}, {{ hedges.correction }}, as.list=T))
```

```{r, echo=FALSE, purl=FALSE}
kable(res$t.test[,c(".y.", "group1", "group2", "n1", "n2","statistic", "df", "p", "estimate",
                    "conf.low", "conf.high", "effsize", "magnitude", "p.signif")], digits = 3)
```
...
cnames <- unique(c("variable", colnames(df)[!colnames(df) %in% cnames], "n", "mean", "median","sd","se","ci"))
